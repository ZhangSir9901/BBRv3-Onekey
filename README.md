...
* 🌐 **内核级 PMTU 动态黑洞探测**：开启 Linux 底层 MTU 探测机制，自动适应各种中转机、加密隧道与老旧路由，不再因数据包过大被静默丢弃。
* 🛡️ **4 重静默冗余防 WAF 拦截**：内置高拟真 Chrome UA 伪装、IPv4 强制降级与临时文件落地校验，静默绕过 Cloudflare 403 阻断，完美适配阿里云香港、AWS 等高危 IP 段。
...
万一遇到由于特定主机商硬件/虚拟化不兼容导致开机死机（概率极低），您只需通过商家网页控制台（VNC/Console）重启服务器，在 GRUB 启动菜单中选择 `Advanced options` ➡️ 选中老内核（如 6.1.x 或 5.x-amd64），即可一键满血救活。
